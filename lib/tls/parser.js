...
Parser.prototype.getPending = function getPending() {
  return Buffer.concat(this.buffers, this.total);
};

Parser.prototype.merge = function merge(size) {
  if (this.total < size)
    return false;
  ...
  // Fast cases
  if (this.buffers[0].length === size) {
    return this.buffers.shift();
  } else if (this.buffers[0].length > size) {
    var r = this.buffers[0].slice(0, size);
    this.buffers[0] = this.buffers[0].slice(size);
    return r;
  }

  var out = new Buffer(size);
  var off = 0;
  for (var i = 0; off !== size && i < this.buffers.length; i++) {
    var buf = this.buffers[i];
    var toCopy = Math.min(buf.length, size - off);

    buf.copy(out, off, 0, toCopy);
    if (toCopy < buf.length)
      this.buffers[i] = buf.slice(toCopy);
    off += toCopy;
  }
  ...
  while (true) {
    ...
    if (!this.header) {
      ...
      this.header = {
        ...
        version: buffer.readUInt16BE(1, true),
    ...
    if (this.header) {
      var buffer = this.merge(this.header.length);
      if (!buffer)
        break;
      ...
      var decrypted = true;
      try {
        // Do not decrypt twice
        var dec = this.state.decrypt(this.rawBody, this.header);
        this.rawBody = dec || this.rawBody;
        this.body = this.rawBody;
      } catch (e) {
        if (this.state.constructor.name === 'Dummy') {
          this.body = null;
          decrypted = false;
        } else {
          err = e;
...
Parser.prototype.parseHello = function parseHello(type, length) {
  ...
  var helloVersion = this.body.readUInt16BE(0, true);
  var random = {
...
Parser.prototype.parseHearbeatExt = function parseHearbeatExt(body) {
  ...
  var mode = constants.heartbeatMode[body[0]];
  if (!mode)
...
Parser.prototype.parseECParams = function parseECParams(body) {
  ...
  var type = constants.curveType[body[0]];
  if (type !== 'named_curve')
    return this._error('Unsupported ECDHE curve type: ' + type);

  var id = body.readUInt16BE(1, true);
...
Parser.prototype.parseSignature = function parseSignature(data) {
  if (this.state.version >= 0x0303) {
    ...
    var hash = constants.hashAlgorithm[data[0]];
    var sign = constants.signatureAlgorithm[data[1]];
    if (!hash || !sign)
      return this._error('Unknown hash or signature algorithm');

    var len = data.readUInt16BE(2, true);
    if (len + 4 > data.length)
      return this._error('Signature content length OOB');
    var content = data.slice(4, 4 + len);
    ...
    var len = data.readUInt16BE(0, true);
    if (len + 2 > data.length)
      return this._error('Signature content length OOB');
    var content = data.slice(2, 2 + len);
...
Parser.prototype.parseECDHEKeyEx = function parseECDHEKeyEx(frame) {
  var params = this.parseECParams(frame.content);
  if (!params)
    return false;

  var point = frame.content.slice(params.size);
  if (point.length < 1)
    return this._error('EC Point length is too small');

  var pointLen = point[0];
  if (pointLen + 1 > point.length)
    return this._error('EC Point length OOB');

  var signature = point.slice(1 + pointLen);
  point = point.slice(1, 1 + pointLen);
  ...
  return {
    params: params,
    rawParams: frame.content.slice(0, params.size + 1 + pointLen),
    point: point,
    signature: signature
  };
};
...
Parser.prototype.parseECDHEClientKeyEx = function parseECDHEClientKeyEx(frame) {
  ...
  var len = frame.content[0];
  if (1 + len !== frame.content.length)
    return this._error('Client ECDHE keyEx length OOB');

  return frame.content.slice(1);
};
