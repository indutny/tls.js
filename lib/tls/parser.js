...
Parser.prototype.parseAlert = function parseAlert() {
  ...
  if (this.body.length === 2)
    this.push(new Alert(this.header, this.body[0], this.body[1]));
  else
...
function Hello(header, type, opts) {
  ...
  this.compressions = new Array(opts.compMethods.length);
  for (var i = 0; i < this.compressions.length; i++)
    this.compressions[i] = opts.compMethods[i];
  this.extensions = {};
...
function CertificateRequest(header, type, types, algorithms, authorities) {
  Handshake.call(this, header, type);
...
function Finished(header, type, verify) {
  Handshake.call(this, header, type);
...
Parser.prototype.parseFinished = function parseFinished(type, length) {
  ...
  this.push(new Finished(this.header, type, this.body));
...
function Heartbeat(header, type) {
  Record.call(this, header);
  this.type = 'handshake';
...
Parser.prototype.parseHeartbeat = function parseHeartbeat() {
  ...
  if (this.body.length === 1 && constants.heartbeatType[this.body[0]])
    this.push(new Heartbeat(this.header, this.body[0]));
  else
