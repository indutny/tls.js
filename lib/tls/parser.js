...
Parser.prototype._transform = function transform(data, encoding, callback) {
  ...
  var dec;
  while (true) {
    var buffer = dec || this.buffer;
    var justDecrypted = !!dec;
    dec = null;
    ...
    if (!this.header) {
      if (buffer.length < 5) {
        if (justDecrypted)
          err = new Error('Invalid record size');
        break;
      }
    ...
    if (this.header) {
      if (buffer.length < this.header.length) {
        if (justDecrypted)
          err = new Error('Encrypted record size OOB');
        break;
      }
      ...
      try {
        ...
        if (!justDecrypted)
          dec = this.state.decrypt(this.rawBody);
      ...
      this.header = false;
      this.body = false;
    } else if (justDecrypted) {
      err = new Error('Unexpected encrypted record end');
    }
    if (err)
      break;
  }
