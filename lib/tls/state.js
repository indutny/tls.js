...
State.prototype.decrypt = function decrypt(body, header) {
  ...
  if (session.info.bulk.cbc) {
    ...
    pre.writeUInt32BE((session.readSeq / 0x100000000) | 0, 0, true);
    pre.writeUInt32BE(session.readSeq & 0xffffffff, 4, true);
    session.readSeq++;
    header.buffer.copy(pre, 8, 0, 5);
    pre.writeUInt16BE(res.length, 11, true);
...
State.prototype.encrypt = function encrypt(body, hdr) {
  ...
  var pre = new Buffer(8 + 1 + 2 + 2);
  pre.writeUInt32BE((session.writeSeq / 0x100000000) | 0, 0, true);
  pre.writeUInt32BE(session.writeSeq & 0xffffffff, 4, true);
  session.writeSeq++;
