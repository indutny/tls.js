...
Session.prototype.computeMaster = function computeMaster() {
  ...
  var randoms = [ this.clientRandom, this.serverRandom ];

  this.masterSecret = this.prf(this.preMaster, constants.prf.master, randoms)
      .read(constants.masterLength);

  var rrandoms = [ this.serverRandom, this.clientRandom ];
  var keyBlock = this.prf(this.masterSecret, constants.prf.key, rrandoms);
  var keyLength = 2 * this.macKeyLength +
                  2 * this.keyLength +
                  2 * this.fixedIVLength;
  keyBlock = keyBlock.read(keyLength);

  var off = 0;
  this.clientWriteMacKey = keyBlock.slice(off, off + this.macKeyLength);
  off += this.macKeyLength;
  this.serverWriteMacKey = keyBlock.slice(off, off + this.macKeyLength);
  off += this.macKeyLength;
  this.clientWriteKey = keyBlock.slice(off, off + this.keyLength);
  off += this.keyLength;
  this.serverWriteKey = keyBlock.slice(off, off + this.keyLength);
  off += this.keyLength;
  this.clientWriteIV = keyBlock.slice(off, off + this.fixedIVLength);
  off += this.fixedIVLength;
  this.serverWriteIV = keyBlock.slice(off, off + this.fixedIVLength);
...
Session.prototype.load = function load(crypto, cipher) {
  ...
  this.info = info;
  this.prf = isTLS12 ? utils.prf(crypto.mac(info.prf)) :
                       utils.ssl3prf(crypto);
  ...
  this.hash = isTLS12 ? crypto.hash(info.prf) : utils.ssl3hash(crypto);
...
State.prototype.clientRSAKeyEx = function clientRSAKeyEx() {
  ...
  var out = new Buffer(2 + content.length);
  out.writeUInt16BE(content.length, 0, true);
  content.copy(out, 2);
