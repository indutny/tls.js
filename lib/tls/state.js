...
Session.prototype.computeMaster = function computeMaster() {
  ...
  this.clientWriteMacKey = keyBlock.read(this.macKeyLength);
  this.serverWriteMacKey = keyBlock.read(this.macKeyLength);
  this.clientWriteKey = keyBlock.read(this.keyLength);
  this.serverWriteKey = keyBlock.read(this.keyLength);
  this.clientWriteIV = keyBlock.read(this.fixedIVLength);
  this.serverWriteIV = keyBlock.read(this.fixedIVLength);
...
State.prototype.decrypt = function decrypt(body, header) {
  ...
  if (session.info.bulk.cbc) {
    // Remove IV
    var iv = out.slice(0, session.recordIVLength);
    ...
    // Remove mac
    var mac = data.slice(-session.macKeyLength);
    var res = data.slice(0, -mac.length);
  } else if (session.info.type === 'stream') {
    var mac = out.slice(-session.macKeyLength);
