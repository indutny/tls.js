...
function Framer(options) {
  ...
  this.version = {
    major: this.options.version.major,
    minor: this.options.version.minor
  };
}
...
Framer.prototype.changeCipherSpec = function changeCipherSpec() {
  this.header('change_cipher_spec', cipherSpec.length);
  this.push(cipherSpec);
};
...
Framer.prototype._handshake = function handshake(type, len, extLen, chunks) {
  ...
  this.header('handshake', head.length + len + extLen);
  this.push(head);
...
Framer.prototype.hello = function hello(type, hello) {
  ...
  // Version
  pre[0] = (hello.version || this.version).major;
  pre[1] = (hello.version || this.version).minor;
  ...
  for (var i = 0; i < hello.cipherSuites.length; i++)
    hello.cipherSuites[i].copy(cipherSuites, i * 2 + 2);
